...
def hamming_within_regions(model, optim, inp_batch, inp_target, iterations, comp_hamming=True, comp_confusion=True):
    ...
    for i in range(iterations):
        ...
        patch = inp_batch[rand_x-25:rand_x+25, rand_y-25:rand_y+25, :]
        patch_target = inp_target[rand_x-25:rand_x+25, rand_y-25:rand_y+25, :]
...
def train(model, optim, criterion, im, encoding, L, args):
    ...
    for epoch in range(args.epochs):
        ...
        if args.confusion_between_region:
            if epoch > args.epochs-2:
                # 25000
                _, confusion_between_region = hamming_between_regions(model, optim, inp_batch, inp_target, 100, comp_hamming=False)

        # mean hamming distance during training
        if args.mean_hamming_in_region:
            if epoch % 250 == 0:
                hamming_in_region, confusion_in = hamming_within_regions(model, optim, inp_batch, inp_target, 250, comp_confusion=False)
...
def main():
    ...
    parser.add_argument('--L', type=int, default=8, help='frequency of encoding')
    ...
    for im in test_data:
        ...
        if args.train_encoding:
            ...
            pe_param_norms, pe_num_patterns, pe_layer_1_norms, pe_layer_2_norms, pe_layer_3_norms, pe_zero_neuron_plot, pe_losses, inp_batch, inp_target, confusion_within_pe, confusion_between_pe, mean_hamming_within_pe, mean_hamming_between_pe = train(model_pe, optim_pe, criterion, im, 'sin_cos', args.L, args)

        averaged_global_confusion_pe.append(confusion_between_pe)
        averaged_local_confusion_pe.append(confusion_within_pe)

    averaged_global_confusion_pe = np.array(averaged_global_confusion_pe)
    averaged_global_confusion_pe = averaged_global_confusion_pe.flatten()

    averaged_local_confusion_pe = np.array(averaged_local_confusion_pe)
    averaged_local_confusion_pe = averaged_local_confusion_pe.flatten()
    ...
    sns.kdeplot(data=averaged_global_confusion_pe, fill=True, label='pe')
    sns.kdeplot(data=averaged_global_confusion_xy, fill=True, label='xy')
    plt.legend()
    plt.show()

    sns.kdeplot(data=averaged_local_confusion_xy, fill=True, label='xy')
    sns.kdeplot(data=averaged_local_confusion_pe, fill=True, label='pe')
    plt.legend()
    plt.show()
